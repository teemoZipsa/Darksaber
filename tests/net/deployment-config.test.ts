...
test('production deployment docs require durable database-backed auth', () => {
    ...
    assert.match(deploymentDocs, /https:\/\/darksaber-azure\.vercel\.app/);
    for (const origin of [
        'https://darksaber-azure.vercel.app',
        'https://darksaber-teemo-zipsa-s-projects.vercel.app',
        'https://darksaber-seonkyuhy-4793-teemo-zipsa-s-projects.vercel.app',
        'https://darksaber.vercel.app',
    ]) {
        assert.match(renderYaml, new RegExp(origin.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    }
