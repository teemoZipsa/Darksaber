...
필드 HUD·미니맵·전투 액션 메뉴는 `public/assets/ui/field-forged/panel.png`를 공유한다. DOM은 `border-image`, Canvas는 `FieldPanelSkin.ts`의 nine-slice로 모서리를 보존한다. 생성 PNG 원본의 투명 알파를 유지했으며 프롬프트는 자산 폴더 `README.md`에 기록했다. 문장은 장식 이미지이고 텍스트·수치·상호작용은 코드로 유지한다.
