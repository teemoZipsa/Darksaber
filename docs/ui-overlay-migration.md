...
필드의 즉시 귀환 버튼과 호출 경로는 제거했다. 마을까지 실제로 이동해 입장하며, 이동 중에는 중지 버튼만 표시한다. 대기 중 상시 탐험 권유 문구는 표시하지 않는다. 시작 대련장 튜토리얼은 별도 흐름으로 유지한다.
...
공격은 원작 동작·타이밍을 재현하는 방향을 우선한다. 원작과 대조하지 않은 타격 연출 강화는 기본 개선으로 취급하지 않는다. 전투 상황 표시는 간결하게 유지한다. 현재 파티원의 테두리는 피격 예고 → 선택 → 조작 순서로 하나만 표시하고, 행동 준비는 기존 행동력 게이지로 나타낸다. 게이지와 겹치던 준비 고리는 제거했다. 공격 동작·판정·효과음 타이밍은 이번 숲 경계/표시 정리에서 변경하지 않았다.
