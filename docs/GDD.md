...
**Genre**: 원작 복각 중심의 필드 탐험·성장 SRPG

**Platform**: Web browser (desktop/mobile)

**As-built baseline**: 2026-09-13
...
- 탐험에는 시간 제한이 없다. 출발한 마을을 포함한 모든 마을로 돌아올 수 있다.
- 획득물과 경험치는 사망·연결 종료·중도 귀환에도 보존된다. 달성한 퀘스트와 보상 골드는 귀환 처리에서 확정한다.
- 안전한 야외에서는 목적지 클릭 한 번으로 일반 AP 이동을 이어 간다. 적 접근 시 연속 이동을 멈추고 전술 조작으로 전환한다.
...
3. 탐험 파티를 결정하고 필드로 출발
4. 월드 필드나 스토리 실내에서 전투·조사·전리품 획득
5. 원하는 마을로 돌아가거나 전투 불능/연결 종료/중도 귀환 결과 확정
...
- 달성한 스토리 목표는 귀환 처리에서 보상 공간을 검증해 확정한다. 사망이나 출발 마을 귀환도 완료를 취소하지 않는다.
...
- 사망에 따른 배낭·장비·경험치 손실과 보험 구매 UI를 제거했다. 부상과 회복 비용, 장비 내구도는 유지한다.
- 진행 필수 퀘스트 보상은 일반 전리품과 분리해 귀환 결과에서 안전하게 확정한다.

저장 계약은 `docs/hub-save-sync.md`와 `server/WorldSessionSaveState.ts`를 따른다.

## 7. UX와 접근성

- 게임 필드는 DPR 대응 Canvas, 필드 상태 HUD와 차단형 메뉴는 React DOM overlay로 렌더한다.
