...
6. 서버 저장을 동기화하고 최근 출격 기록과 몬스터 도감을 검토한 뒤 다음 출격 준비
...
- 마을 퀘스트 게시판과 J키 저널에서 캐릭터별 최근 출격 20건의 결과·경로·시간·처치·손익을 확인한다.
- 같은 화면에서 전체 61종 몬스터의 미발견·조우·토벌 진행을 검색·계열 필터로 확인한다. 감지 범위 조우는 생태 정보를, 첫 서버 확정 처치는 기준 전투 수치를 해금한다.
