...
- Network world session routing is realm + raid-instance based, not account-hash based. Default solo joins use the legacy `realm:primary` key; party/raid flows can pass `requestedRaidInstanceId` and route every member to `realm:raid:<id>`.
