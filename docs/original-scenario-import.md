...
- Episodes 4-6 are outdoor/field scenarios. Their v1 event data keeps the original `DEO` entry dialogue and `evt` reward/objective events, but field placement is adapted to the current world map instead of forcing original coordinates. Inspect events are projected from original map coordinates onto tiles around each current scenario entrance.
