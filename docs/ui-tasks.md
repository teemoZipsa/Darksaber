...
- [x] 시작 튜토리얼: 캐릭터 생성 확인 후 `src/ui/react/tutorial/*`의 모의 전투 튜토리얼을 거쳐 완료/스킵 모두 기존 캐릭터 생성 완료 경로로 진입. 튜토리얼 중 새로고침/언마운트 시 캐릭터는 아직 확정되지 않았으므로 캐릭터 생성부터 다시 시작하는 것이 의도된 동작.
