# UI 인수인계 (Codex 이어받기용) — 2026-08-28 기준

> 캔버스 UI → React DOM 오버레이("풀다크 Darkest Dungeon") 이전은 주요 패널 기준 완료 상태이며,
> 독립 차단형 패널은 합체 신전 하나가 남아 있습니다.
...
| 탐험 결과 | `src/ui/react/result/RaidResultPanel.tsx` | 보상·파티·목표 전체 표시, 단일 스크롤, 터치·키보드 귀환 |
...
남은 독립 차단형 이전 대상: `src/ui/FusionTempleUI.ts`.

- [x] 필드·인벤토리 진단 결함 5종 수정 (2026-09-14): 정렬을 임시 배치에서 먼저 검증해 실패 시 아이템·수량·좌표를 모두 보존한다. 장착 시 레벨·전직·계열을 확인하고 서버도 새 장착 저장을 검증하며, 기존 저장 장비는 삭제하지 않는다. 로컬/온라인 캐릭터가 같은 스프라이트 연결을 사용해 다른 플레이어와 전직·합체 외형을 표시한다. 해안은 원작 조각을 동일한 2×2 구성/배율로 이어 좁은 물길과 일반 강둑 사이의 크기 불일치를 제거한다. 드롭 미리보기와 실행은 공통 배치 판정을 사용해 스택 합치기·가득 찬 스택·미확정 전리품 안내를 일치시킨다.

검증: 단위 검사 943개 통과·실DB 연동 1개 스킵. 실제 소실 배치의 보검·스택·내구도·인스턴스 보존, 1레벨 캐릭터의 106레벨 무기 장착 차단, 서버 저장 우회 차단, 반복 스냅샷에서 이미지 재사용·전직 갱신 검증. 데스크톱/모바일에서 스택 드래그·장착 거부·부르고스 해안 맞닿는 픽셀·신규 8직업 온라인 스프라이트 로드 확인. typecheck·lint·클라이언트/서버 빌드·필수 자산 검사 통과.

- [x] 아이템 이모지·문자 그림 보완: 전체 375종 중 그림이 없던 200종에 연결. 154종은 원작 스프라이트 재사용, 46종은 내장 이미지 생성으로 만든 13개 도트 그림 사용(룬·향 계열 공유). 기존 원작 지정 175종과 게임 수치는 보존. 새 그림은 32px RGBA·사방 3px 여백으로 정규화하고 인벤토리·상점·툴팁·Canvas 도구 목록에서 공통 아틀라스 선택. 개별 PNG·아틀라스·프롬프트·재생성 스크립트 동봉(`docs/item-artwork.md`).

검증: 375종 셀 범위·실제 픽셀·투명 여백·원작 좌표 보존 검사, 전체 단위 검사 931개 통과·1개 스킵. 데스크톱/모바일 인벤토리·툴팁·상점·375종 Canvas 픽셀 일치 및 기존 장착/해제 검사 4개 통과. 실제 배낭 캡처와 수리 키트 화면 내 표시 확인. typecheck·lint·build·필수 자산 검사 통과.

- [x] 8개 핵심 직업 개별 외형·필드 애니메이션: 그랜드소드·그랜드아처·그랜드에이지·비약사·현자·필로소퍼·그랑힐러·힐러마스터를 서로 다른 인물, 색상, 장비 실루엣으로 ImageGen 재제작. 각 직업에 128×128 초상과 32px 3열×6행 시트를 설치해 4방향 걷기 및 남/북 액션을 실제 필드 로더에 연결했다. 생성 배경과 논리 셀을 넘은 포즈는 전용 연결 성분 정규화 스크립트로 투명화·보존하며, 프롬프트와 규격은 `docs/generated-character-sprites.md`에 기록했다.

검증: 전용 검사 2개에서 8개 초상·144개 프레임의 크기, RGBA, 전경, 투명 테두리, 프레임 변화, 외형 비중복 및 직업·티어 런타임 매핑 통과. 전체 단위 검사·typecheck·lint·build·필수 자산 검사 통과. 8개 캐릭터의 남쪽 걷기와 직업 액션을 로컬 GIF로 프레임 재생 확인.

- [x] 필드 캐릭터 대기 모션: 현재 시트에 별도 대기 셀이 없어 걷기 프레임을 제자리 재생하지 않고, 남쪽·북쪽·좌우의 중립 프레임을 그대로 유지한 채 1픽셀 범위의 느린 호흡을 적용. 12단계·2.4초의 정수 픽셀 패턴을 캐릭터 ID별 위상으로 어긋나게 해 파티가 동시에 들썩이지 않는다. 파티와 튜토리얼 인물의 완전 정지 상태에만 재생하고 이동·공격·마법·피격·쓰러짐 중에는 끄며, 적과 정보창에는 적용하지 않는다. 설정의 `모션 줄이기`가 켜지면 대기도 완전히 정지한다.

검증: 전용 검사 3개에서 전체 -1~1px 상태, 2.4초 정확한 순환, 캐릭터별 안정적 위상 분리, 모션 줄이기 및 비정상 시간 정지를 확인. 필드 단위 검사 499개와 전체 단위 검사·typecheck·lint·build·필수 자산 검사 통과. 실제 8개 직업의 남쪽 중립 프레임과 동일한 위상 계산으로 만든 로컬 움직임 미리보기는 `test-results/field-character-idle-motion.gif`.

- [x] 필드 즉시 귀환 제거·일반 조작음 정리: HUD 귀환 버튼과 즉시 종료 호출 경로 제거, 상시 탐험 권유 문구 숨김. 누락된 서버 플레이의 마을 도착 처리를 연결하고 서버가 확인한 좌표로 입장·저장 요청. 마을/필드 전환 효과음 제거, 전리품 창에 잘못 연결된 원작 04 대신 짧은 UI 열기음 사용, 클릭음 감쇠 및 이동 성공음 제거. 오디오 잠금 해제 이전 효과음 요청은 즉시 폐기.

검증: 초기 전체 단위 검사 925개 통과·1개 스킵. 마을 도착 연결 후 관련 검사 559개 중 기존 스냅샷 테스트의 도착 컨트롤러 대역 누락 3개를 보강하고 해당 28개 검사 재통과. 브라우저 36개 중 마을 도착 누락 2개를 재현·수정한 후 데스크톱/모바일 서버 이동·입장·저장·재진입, 조작음 호출, 오디오 잠금 해제·음소거·장면 전환 6개 재검사 통과. typecheck·lint·build·필수 자산 검사 통과.

캔버스 유지(이전 대상 아님): 월드 렌더, 적 체력바, 플로팅 데미지, 전술 마커, 방사형 액션메뉴 등 **월드/카메라 좌표 HUD**.

---

## 현재 후속 점검

- [x] 부르고스 성 남서쪽 강가의 누락된 접경 수정: 첨부 화면을 원작 01hmap (9, 98)과 대조해 같은 지점 재현. 투명한 해안 바깥에 물 바탕이 비치던 파란 네모를 실제 인접 육지 합성으로 제거. 원본 해안의 큰 바위·곡선은 유지하고, 원작 셋에 없는 좁은 수로·웅덩이·복합 모서리만 3×3 원작 조각을 합성해 연결. 깊은 물도 같은 해안 처리 적용. 일반 지면 경계에서 제외됐던 벽·입구의 돌 그림을 같은 시각 계열로 연결해 잘못된 초록 바탕과 끊긴 돌 조각 제거. 충돌·이동 비용·맵 데이터 변경 없음.
  - 검증: 수정 전 해안 바깥 수면 표본 292개, 해당 현장 벽 아래 녹색 누출 23,175픽셀을 실패로 재현한 후 모두 0 확인. 데스크톱/모바일 해안 32형태·청크 접경·실제 부르고스 현장·물결 12프레임·숲·설원·일반 지면 E2E 12개, 필드 단위 검사 494개, typecheck·lint·build·필수 자산 검사 통과. 같은 위치 전후 비교 `test-results/coastline-comparison.html`.

- [x] 운영 배포 전 개발자 시작 E2E 시간 예산 보정: 데스크톱에서 마을·전리품·튜토리얼까지 여섯 번 전체 페이지를 로드하는 검사가 합산 30초 제한으로 마지막 진입 전에 종료됨을 Playwright trace로 확인. 개별 준비 조건은 유지하고 테스트 전체 제한만 60초로 조정. 검증: 해당 검사 데스크톱/모바일 재검사 및 전체 타입 검사 통과. 배포 대상 게임 코드 변경 없음.

- [x] 원작 숲 경계와 간결한 전투 표시: 숲 왼쪽 가장자리에 오른쪽 칸이 섞이고 오른쪽에는 내부 칸이 쓰이던 매핑을 수정(왼쪽 5, 오른쪽 7). 일반 외곽은 원본 한 칸의 수관·밑동을 보존하고, 없는 오목 모서리는 원본 풀 경계 알파와 숲 잎 그림으로 합성. 숲 수관이 길·모래·돌·눈보다 위에 그려지도록 시각 순서만 수정해 경계에서 나무가 사각형으로 잘리지 않게 함. 전투는 겹친 선택/조작/위험 테두리를 하나로 정리하고 준비 고리 제거, 기존 행동력/체력 게이지 유지. 행동 배지의 이전 Canvas 경로가 함께 칠해지던 오류 수정. 공격 동작·판정·타이밍 변경 없음.
  - 검증: 원본 PNG를 쓰는 좌우 경계 32개 좌표와 네 방향 오목 모서리, 숲 포함 6종 지형×256 연결 형태, 청크를 가로지르는 오솔길/공터 및 실제 숲 마을 도로 렌더 확인. 숲/물/눈/일반 경계·전투 표시·공격/마법/도구/방어/휴식/집결 조작 데스크톱·모바일 E2E 12개 통과(실제 마을 캡처 로딩 조건 보강 후 숲 2개 재통과). 필드 테스트 489개, 전체 타입 검사·lint·클라이언트 빌드 통과.

- [x] 설원 바탕 오지정 수정: 2026-06-01 `fd35075`부터 눈으로 지정되어 있던 `mdsr0` 448/452(흰 배경 위 파란 조각)를 제거하고 프로젝트의 기존 `snow.png`를 연결. 해당 칸의 원작 용도 및 원작 설원 세트는 확인되지 않았으므로 원작 설원 복원으로 표시하지 않음. 텍스처를 월드 좌표에 맞는 8×8칸 패치와 32px 도트로 렌더링해 매 칸 동일 조각 반복을 없애고 기존 경계 마스크 유지. 설원 진입 시만 지연 로드하고 로드 완료 때 청크와 눈 경계 캐시를 갱신해 흰 임시 바닥이 남지 않도록 수정.

검증: 데스크톱/모바일에서 설원 로딩을 지연시킨 후 새로고침·markDirty 없이 흰 바탕과 경계가 정상 질감으로 바뀌는 검사 통과. 지형 접경·1,280개 이웃 조합·물결 회귀 포함 브라우저 6개, 필드 단위 검사 488개·typecheck·lint·build 통과. 비교 이미지 `test-results/snow-ground-comparison.png`.

- [x] 지형 접경 연결 개선: 모든 육지 아래에 풀을 깔던 경계 처리를 실제 주변 지형의 우선순위 합성으로 교체. 모래–길·모래–돌·돌–눈 사이의 잘못된 초록 띠를 제거하고 도로–마을 포장을 같은 연결 면으로 처리. 평지에 섞이던 절벽/해안 조각을 제외하고 숲 가장자리 방향을 교정. 원작 조각을 네 모서리로 조합해 안쪽/바깥쪽 모서리·좁은 길·갈림길을 연결하며, 눈에는 원작 지면의 불규칙한 알파 경계를 적용. 충돌·이동 비용·맵 데이터는 그대로 유지.

검증: 수정 전 접경에서 녹색 누출 픽셀 42,150개로 재현한 검사가 수정 후 0개로 통과. 데스크톱/모바일에서 5종 지형×256가지 이웃 조합의 연결과 청크 접경을 검사하고 물결 12프레임·해안 고정·청크 재생성 회귀 검사까지 총 4개 브라우저 검사 통과. 필드 단위 검사 488개·typecheck·lint·build 통과. 비교 이미지는 `test-results/terrain-boundaries-comparison.png`(위에서 풀–길, 모래–길, 모래–돌, 돌–눈).

- [x] 원작 물 타일 기반 잔물결 애니메이션: 불투명한 물/깊은 물 타일 두 변형을 완만하게 섞고 원본 셀 내부에서만 1픽셀 물결을 순환. 12프레임·1.92초 주기이며 해안과 대각선 섬 가장자리 타일은 고정. 화면에 보이는 청크의 수면만 160ms마다 갱신하고, 최대 48개의 32×32 프레임을 공유해 청크 전체 재생성과 추가 대형 버퍼를 피함. 모든 청크가 같은 시계를 사용해 로딩·이동 시 위상 유지.

검증: 데스크톱/모바일에서 12개 전 프레임의 물·깊은 물·색상 전환·청크 경계 움직임, 투명/갈색 바위 픽셀 0개, 해안/육지 픽셀 변경 0개 확인. 첫 프레임으로 정확하게 순환하고 같은 시간의 청크 재생성 전후 픽셀이 일치하며, 애니메이션 중 지형 이웃을 재계산하지 않는 회귀 검사 통과. 필드 단위 검사 483개·typecheck·lint 통과. 실제 렌더 프레임으로 만든 로컬 움직임 미리보기는 `test-results/water-animation.gif`.

- [x] 물 바탕 타일 오류 수정: `mdsr0`의 투명한 대각선 해안 조각 145/146을 물 바탕 후보에서 제외하고, 불투명한 원작 물 타일 131/132만 사용. 강·바다 내부의 검은 구멍과 바위 조각, 깊은 물 경계에 섞이던 해안 조각을 제거.

검증: 실제 원본 PNG와 Chunk 렌더러로 강·섬·깊은 물·두 청크의 경계를 구성한 브라우저 회귀 검사에서 수정 전 투명 픽셀 489,595개로 실패 재현. 수정 후 데스크톱/모바일 모두 물 내부의 투명 픽셀·갈색 바위 픽셀 0개, 렌더 이미지 확인. 타일 자산 로딩 단위 검사·typecheck·lint 통과. 현재 물 지형은 청크에 정적으로 캐시되며 이번 문제는 애니메이션이 아닌 바탕 타일 선택 오류였다.

- [x] CC0 보충 오디오: Kenney 효과음 15개와 Juhani Junkala JRPG 음악 4곡을 확보해 지형별 발소리·대장간·일지·완료/패배 알림·마을/사막/일반 폐광에 연결. 원작 핵심곡·전투 효과음을 유지하고, 추가 음원 음량을 맞춘 뒤 개별 출처·라이선스·해시·가공 내역을 동봉.

검증: 전체 단위 테스트 918개 통과·1개 건너뜀, typecheck·lint·build·verify:assets 통과. 데스크톱/모바일 2개 브라우저 검사에서 전체 음원 디코딩·음소거 복구·마을 3종의 실제 입장 상태와 BGM 자동 전환·기존 마을곡 대비 추가 BGM 음량 차이 확인. 추가 파일의 합계는 약 5.97 MB. 로컬 비교 재생 페이지는 `.runtime/free-audio/preview.html`이며 내려받은 전체 팩과 검토 파일은 커밋에서 제외.

- [x] 원작 오디오 연결: WAV 효과음 20개를 전투·마법·회복·전리품·상점에 연결하고, 중복 제외 MIDI 7곡을 타이틀·마을·필드·전투·시나리오에 자동 배치. 첫 입력 오디오 잠금 해제, 음소거 중 음악 전환, 중복 로딩·연속 효과음·늦은 음악 로딩 충돌을 정리. 추정 배치와 MIDI 음색 한계는 `docs/original-audio.md`에 기록.

검증: `npm test` 916개 통과·1개 건너뜀·실패 0. typecheck·lint·build·verify:assets 통과. 데스크톱/모바일 브라우저 2개 검사에서 모든 원본 오디오 디코딩·출력 신호·첫 입력 잠금 해제·음소거 상태의 마을→필드 전환과 재활성화 확인. 이전 행동 메뉴 변경으로 사라진 번역 호출 1건은 i18n 검사 허용 목록에서도 제거.

- [x] 원작 아이콘 중심 행동 메뉴 복원: 작은 화면의 카드·연결선·고리와 모든 화면의 단축키 배지·상시 설명 제거, 8방향 원작 아이콘·터치 영역 유지, 마법 선택 배지 제거. UI 지침을 원작 표현 우선으로 수정. UI 배율과 카메라 줌을 반영해 아이콘 크기·간격·클릭 영역을 캐릭터와 같은 월드 타일에 맞췄으며, 메뉴에 겹치는 사냥터 이름 표식은 숨김.

검증: 관련 단위 검사 40개, typecheck·lint·build 통과. 320·390·516px/120% UI 배율에서 실제 캐릭터 좌표와 8개 아이콘 중심·크기를 비교하는 브라우저 검사 통과. 데스크톱 전투 검사 통과. 모바일 전투의 공격 후 피해 감소 검사는 1회 실패했으며 코드 변경 없이 단독 재실행에서 공격·마법·도구·방어·휴식·집결 모두 통과했다. 이 검사는 확률 명중을 사용하므로 좌표 검사와 별도로 기록한다.

- [x] 캐릭터 주변 8방향 액션 아이콘 배경 투명화: 검은 사각 패널·장식 테두리를 제거하고 단축키·호버 표시·클릭 영역 유지. 실제 브라우저 화면 확인, 관련 단위 검사 13개·데스크톱/모바일 전투 검사 2개·typecheck 통과.

- [x] 탐험 결과창 DOM 전환: 잘리던 보상·목표·파티 상태 전체 표시, 기존 어두운 프레임과 안전 여백 적용, 작은 화면 단일 스크롤·고정 확인 버튼, 터치·Enter·Space·Escape·포커스 순환과 중복 정산 방지 검증. 일반 계정 접속 후 개발자 메뉴가 마을 탭을 가리던 문제도 함께 수정.

검증: 코드 테스트 907개 통과·환경 의존 1개 스킵, typecheck·lint·build 통과. 전체 브라우저 76개 중 75개 통과 후 모바일 개발자 메뉴 가림을 발견·수정했고, 결과창·개발자 메뉴·일반 계정 귀환의 영향 경로 12개를 재검증해 통과했다. 320px·390px·960px·1280px/120% UI 배율에서 한글·영문, 32개 보상·10개 목표·긴 이름·큰 숫자의 여백과 마지막 항목까지의 스크롤을 확인했다.
...
- [x] 카오시아 근교 사냥 동선: 출구 근처의 단계별 고정 전투 3곳, 다음 사냥터 안내와 금색 표식, 야외 경로의 마을 오진입 방지, 서버 재생성·저장 및 데스크톱·모바일 검증.

- [x] 원작 복각 중심 필드 전환: 탈출 제한·획득물 손실 제거, 안전한 야외 연속 이동과 귀환, 필드 DOM HUD, 저장 및 데스크톱·모바일 검증.

2026-09-13 예외 기록: 사용자 승인 “탈출 규칙을 빼고 원작 복각 중심으로 가자”에 따라 필드 HUD에 필요한 `OverlayRoot`·`UiStore` 연결을 수정했다. 기존 공용 디자인 토큰은 유지했다. 진단과 범위는 `docs/field-restoration.md`를 따른다.


- [x] README·프로젝트 로드맵 현행화. 플레이 가능한 기준선, 로컬 실행·검증, 현재 제한을 README에 정리하고 안정성 게이트→플레이 완성도→콘텐츠 확장 순서와 완료 조건을 `docs/roadmap.md`에 고정한다.
- [x] 최근 출격 기록 V1. 캐릭터별 최근 20회의 생환·전멸·실종·중도 복귀 결과를 로컬/서버 최종 저장에 기록하고, 마을 퀘스트 게시판과 J키 저널에서 경로·시간·처치·확보/손실·골드 요약을 한·영으로 확인할 수 있게 한다.
- [x] 몬스터 도감 V1. 전체 61종 원장을 미발견·조우·토벌 단계로 표시하고, 감지 범위 진입과 서버 확정 처치를 캐릭터별로 저장해 생태·출현지·전투 역할·토벌 수치를 마을 퀘스트 게시판과 J키 저널에서 검색·계열 필터와 함께 한·영으로 확인할 수 있게 한다.
- [x] 정예 현상수배 게시판 V1. 마을마다 결정적으로 갱신되는 의뢰서 3장 중 하나를 뜯어 수락하고, 전용 정예의 특성 2개·위험 목표·개인 전리품 증표를 서버 권위로 추적해 다른 마을 생환 시 기본 현상금과 조건부 보너스를 지급한다.
- [x] 정예 현상수배 추적 V2. 의뢰서의 마지막 목격 방향과 특성 효과를 확인하고, 실제 도달 가능한 수색 구역에서 순서대로 단서 2개를 조사해야 개인 정예 은신처와 표적이 공개되며 진행·상호작용·재접속·지도 표시는 서버 권위와 로컬 개발 레이드에서 동일하게 유지된다.
- [x] 필드 조작감 V1. 메뉴 바깥 클릭·ESC·우클릭은 턴을 종료하지 않고 SPACE만 명시적 대기로 처리하며, 준비된 현재 캐릭터의 지면 클릭을 즉시 이동으로 연결하고 실제 가중 경로·지형 부담·이동 후 행동력·실패 복구 피드백을 표시한다.
- [x] 필드 이동감 V1. 연속 타일 이동의 중간 정지를 제거하고 실제 보간 좌표 카메라 추종·작은 데드존·진행 방향 예측·모션 감소 대응과 지형별 절차형 발소리를 적용한다.
- [x] 모바일 필드 HUD 반응형·입력 우선순위 보강. 390px 및 120% UI 배율에서 레이드 배너·캐릭터·선택 정보·미니맵을 겹치지 않게 재배치하고, 최상단 패널부터 입력을 소비해 숨은 행동 슬롯 오작동을 막으며 터치로 전체 지도를 열고 축소 지도로 복귀할 수 있게 한다.
- [x] 설정창 스크롤·반응형 UX 보정. 낮은 화면에서 패널/본문 스크롤바가 이중으로 생기지 않게 단일 스크롤 구조로 바꾸고, 좁은 화면의 볼륨 슬라이더와 단축키 목록을 읽기 쉬운 세로 흐름으로 재배치하며 토글을 키보드 조작 가능한 네이티브 버튼으로 변경한다.
- [x] 거대한 대륙 내부 수계 밀도 보강. 기존 두 강에 권역별 지류 8개와 불규칙 호수 10개를 추가해 북부 사막·서부 숲·중앙 평야·동부·남부의 넓은 육지를 자연스럽게 분절하고, 도로·다리와 마을·신전·던전 입구의 접근성은 유지한다.
- [x] 기본 공격 사거리를 직업 임시값이 아니라 장착 무기 데이터로 판정. 매지션의 지팡이·단검 기본 공격은 1칸 단일 대상으로 제한하고, 활·창 등 원거리 무기는 각 장비의 공격범위를 로컬 타게팅과 서버 권위 판정에 동일하게 적용한다.
- [x] 직업별 마법 목록 중복 정리. 원작 주문이 같은 역할의 직업 전용 임시 주문을 승계하고 `파이어볼→파이어`처럼 상위 주문이 하위 주문을 교체하도록 해, 매지션을 포함한 모든 직업·티어의 습득 목록에 같은 주문명이나 실질 중복 변형이 함께 노출되지 않게 한다.
- [x] 필드 몬스터 자연스러운 등장 보강. 주기 갱신과 5분 리젠은 파티 시야 밖에서만 허용하고, 현재 청크 강제 생성 및 마을·해양 지형을 초원으로 우회하던 폴백을 제거해 이동 중 화면 안이나 마을 바로 옆에 무리가 갑자기 나타나지 않게 한다.
- [x] 서버 권위 출격 준비 복원. 인증 저장본의 활성 파티 장비·소켓 보너스, 마법 슬롯, 스킬 강화, 식사 상태와 배낭·장비 총중량을 서버 전투 상태로 재구성하고 식사 소비·만료 및 장착 소켓 재접속 복원을 보장한다.
- [x] 서버 권위 네트워크 처치 경험치 저장. 물리·마법·광역 처치가 공용 성장 규칙으로 경험치·레벨·티어·문장·능력치를 갱신하고, 저장본·재접속 스냅샷·인증 캐릭터 메타데이터에 반영하며 사망 초기화와 현세/마스터 성장 제한을 동일하게 적용한다.
- [x] 서버 권위 네트워크 레이드 편집 잠금. 출격 중 파티 편성·장비·소켓·마법 설정 변경을 입력과 UI 모델 계층에서 차단하고, 서버가 확정하는 레이드 전리품 회수·롤백은 유지한다.
- [x] 서버 권위 네트워크 부상 저장. 소유 대원의 down 이벤트·사망 스냅샷과 서버 적·저주 다운 판정을 로스터 `injured` 상태에 연결하고, 저장·재접속·출격 복원·치료비 검증을 서버 권위로 보장한다.
- [x] 손실 없는 허브 저장 큐. 저장 중 추가 변경을 세대별 후속 flush로 보존하고, 일시 장애 백오프·온라인 재시도·오류 표시·출격 전 drain·페이지 종료 keepalive·캐릭터 세션 격리를 보장한다.
- [x] 생성 성공 후 선택 실패 중복 방지. 서버 생성이 확정된 캐릭터를 선택 요청 전에 계정 목록에 반영해 자동 선택이 실패해도 생성 폼이 아닌 선택 화면에서 재시도하게 한다.
- [x] 영어 모드 런타임 현지화 완결. 상점·장비·마법·도구·월드·스토리·적·전리품·스토리 동료 표시를 현재 언어로 통일하고, 운영 스냅샷·구버전 저장본·열린 마을·튜토리얼을 포함한 언어 전환 시 캐시된 표시명도 즉시 갱신한다.
- [x] 필드 밸런스 운영 지표 보강. 출격 후 첫 교전 시간, 15초 이상 분리된 교전 횟수·간격, 획득·생환 전리품 수량, 초반·저·중·고위험·시나리오 지역별 처치, 적·저주·시간초과·수동 이탈 사망 원인을 레이드 결과에서 저카디널리티 Prometheus 지표로 집계한다.
- [x] 장시간 필드 이동 캐시 상한 보강. 화면 보존 범위를 벗어난 지형 청크를 제거할 때 나무·다리 장식, 지면 디테일, 길가 랜드마크 캐시도 함께 비워 장거리 이동 중 메모리가 계속 증가하지 않게 한다.
- [x] 길가 소형 랜드마크 조사 상호작용. 야영지·유적·표지석·늪 토템을 조사 행동 대상으로 노출하고 약초·교역품·골드·해독제 또는 함정을 서버가 거리·행동력·중복·보관 공간 검증 후 확정하며, 재접속 스냅샷에도 조사 완료 상태를 유지한다.
- [x] 소모품·재료 자동 스택 보강. 소모품은 최대 10개, 재료는 최대 20개까지 동일 칸에 합치고 장비·룬·젬은 개별 인스턴스를 유지하며, 저장 정규화와 서버 복구 지급도 같은 상한을 사용한다.
...
- [x] 필드 대형 지형지물 V1. 투명 배경 쓰러진 고목·바위·무너진 돌벽·버려진 수레를 지형별로 희소하고 결정적으로 배치하고 도로·마을·신전·던전을 피하며, 몸체 타일만 충돌시키고 전후 가림을 분리 렌더링한다.
- [x] 필드 지형별 바위 변형 V2. 돌밭·설원·사막·늪 전용 투명 바위 군집을 추가하고 일반 바위는 초원·숲으로 제한하며, 각 변형을 희소하게 결정 배치하고 중심 몸체 한 칸만 충돌시킨다.
...
- [x] 필드 전투 미세 타임라인 추가. 서버 권위 판정은 즉시 유지하면서 공격 예비동작, 접촉 순간 피해 표시·효과·히트스톱, 방향성 피격 반동, 회복 순으로 표현하고 연출 중 중복 입력을 차단한다.
...
- [x] 레이드 랩 다인 파티 조합 매트릭스 추가. 실제 저장본·서버 join 경로로 1~3인 파티와 multi-ready 정책을 pairwise/holdout seed에서 스윕하고, 소유권·중복 ID·보상 1회·결과 1회·결정적 digest를 검증한다.
- [x] Playwright 월드 단축키 준비 조건 보강. `state === WORLD`뿐 아니라 전환 페이드의 입력 잠금이 풀린 뒤 KeyI/KeyK를 보내 데스크톱 E2E의 첫 키 유실을 막는다.
- [x] DOM UI 안전성·접근성·반응형 UX 후속 보강. 공용 확인 대화상자와 설정 변경 이력, 키보드 조작·포커스 표시, 모션 감소, 빈 목록·로딩 상태, 소형 화면 레이아웃을 정리하고 전체 지도 캐시를 미니맵 표시 중 사전 생성한다.
- [x] 인벤토리 키보드 조작 보강. 아이템을 포커스할 수 있게 하고 Enter/Space로 배낭·창고 이동 또는 장착·해제를 실행하며, 이동 뒤에도 해당 아이템에 포커스를 유지한다.
- [x] 창고·캐릭터 패널 반응형 보강. 좁은 화면에서는 15×10 창고 그리드만 내부 스크롤시키고 관리열을 세로로 접으며, 캐릭터 정보·장비·스탯 열을 화면 폭에 맞춰 재배치한다.
- [x] 파티 빈 슬롯 키보드 배치 보강. 다음 빈 슬롯을 포커스 가능한 버튼으로 제공하고 Enter/Space로 대기 캐릭터를 배치한 뒤 새 활성 카드에 포커스를 유지한다.
- [x] 모바일 방사형 행동 메뉴 아이덴티티 보존. 배우 중심의 8방향 링·스포크·정보 페탈로 행동명/비용/불가 사유를 상시 표시하고, 320/390px·UI 120% 화면 클램핑과 전체 지도 우선 입력을 함께 보강한다.
