...
- `AUTH_ALLOWED_ORIGINS`: pinned in `render.yaml` to the canonical Vercel
  production origin `https://darksaber-azure.vercel.app` and its active aliases
...
3. Confirm `AUTH_ALLOWED_ORIGINS` contains `https://darksaber-azure.vercel.app`
   and every active production alias from `vercel alias ls`. If an alias changes,
   update `render.yaml` in the same commit.
