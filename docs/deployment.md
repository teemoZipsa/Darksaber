...
- `AUTH_ALLOWED_ORIGINS`: pinned to the canonical Vercel production origin
  `https://darksaber.vercel.app` in `render.yaml`
...
3. Confirm `AUTH_ALLOWED_ORIGINS` is `https://darksaber.vercel.app`. If the
   canonical client origin changes, update `render.yaml` in the same commit.
