...
확정한다. 저장소 전체의 작업 순서는 [프로젝트 로드맵](roadmap.md)을 따른다.
...
현재 실내화 대상(1, 2, 3, 7, 13, 18~31화)은 모두 별도 실내맵 진입 흐름과
...
23~31화 후반 실내는 원본 `MAP/NNset.arc`/`NN.ai`와 hmap 원장을 기준으로 보스·경비 배치와
유물 회수 이벤트를 반영하고, `NN.DEO`/`NN.DEE`가 있는 화는 해당 발표 포커스까지 별도 실내맵에 반영한다.
...
1. 각 실내맵의 목표 HUD와 입장/복귀/전리품 로그가 자연스럽게 보이는지 실제 플레이로 확인한다.
   ...
   - 완료: 같은 DEV 직접 진입 경로를 구현된 스토리 전체로 확장했다. `npm run dev:raid:story -- storyNN` 또는 URL의 `devScenario=storyNN` 형식으로 모든 구현 시나리오를 개별 점검할 수 있고, 지원 에피소드는 `story-scenarios.json` 데이터에서 결정된다. 자동 테스트는 실내 에피소드가 로컬 실내 던전으로 시작되고 필드/비공정 에피소드가 월드맵 목표 상태로 시작되는지, DEV 상태가 각각 `interior-ready` 또는 `scenario-ready`로 표시되는지 검증한다.
...
- `npm run verify:story`: 필수 렌더/BGM/font 자산 참조와 1~31화의 선언된 원본 `Wlib`/`Glib`/`MAP` 파일과 `set.arc`
  ...
  실내 접근성/23~31화 후반 원본 AI·MRC 및 존재하는 DEO·DEE 발표 포커스/hmap 재생성/후반 봉인 권역 바이옴·위험도/
