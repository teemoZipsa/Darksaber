...
원작 복각을 중심으로 필드 탐험·전투·성장을 구현하는 웹 SRPG입니다. TypeScript 기반 Canvas 필드와 React DOM UI, Node.js 권위 서버를 함께 사용합니다.
...
- 시간 제한 없는 탐험, 출발 마을을 포함한 자유로운 귀환, 사망·중도 복귀에도 획득물과 경험치 보존
- 안전한 야외의 연속 클릭 이동, 적 접근 시 중지, 필드 상태 HUD
- 전리품 자동/수동 획득, 격자 인벤토리와 소켓
...
- 캐릭터별 몬스터 도감: 전체 61종의 미발견·조우·토벌 진행, 생태·출현지·역할·전투 수치, 검색·계열 필터
- 계정·캐릭터 저장, 서버 권위 전투/전리품/퀘스트 결과, 재접속과 진행 중 레이드 복구
- 한국어/영어 전환과 데스크톱·모바일 입력/레이아웃

2026-09-13 전환의 진단과 구현 범위는 [필드 복각 기준](docs/field-restoration.md)에 기록했습니다.
...
콘텐츠를 바로 확인하려면 Vite만 여는 로컬 미리보기 명령을 사용합니다. Windows에서도 같은 명령을 사용합니다.
...
시작 화면의 **개발자 미리보기 → 마을 / 필드 탐험 / 튜토리얼**과 위 명령은 서버 없이 테스트 캐릭터로 시작합니다. 로컬 마을에서도 필드 진입·귀환·재진입이 가능하며, 진행은 새로고침하면 초기화됩니다. 기존 로컬 저장본과 서버 캐릭터는 변경하지 않습니다.

**상황별 테스트**는 적 추격·전리품·전투·스토리의 재현 장면입니다. 일반 플레이와 장비·퀘스트 상태가 다릅니다. `/?devStart=raid&devScenario=loot`처럼 예전 주소로 진입해도 테스트 장면은 항상 로컬에서 실행합니다.

**서버 접속 테스트**는 `npm run server`가 실행 중일 때 사용합니다. `/?devStart=town`과 `/?devStart=raid`는 전용 개발 계정으로 접속하며, 실패하면 로컬 전환 안내를 표시합니다. 화면 아래 DEV 표시를 열면 현재 모드와 개발자 메뉴로 돌아가는 링크를 확인할 수 있습니다. 일반 계정 로그인은 별도입니다.
...
- 주요 메뉴와 탐험 결과는 React DOM으로 이전됐지만 합체 신전은 아직 Canvas 차단형 패널입니다.
